...
class StaticMplCanvas(MyMplCanvas):
    ...
    def dispImg(self,im,clut,autoscale=True,cuts=[0.0,1.0]):
        ...
        if autoscale:
            ...
            self.axes.imshow(im,clim=(minC,maxC),cmap=clut)            # Autoscale

        else:
            self.axes.imshow(im,clim=(cuts[0],cuts[1]),cmap=clut)     # Can use e.g. clim=(0.2, 0.5), cmap="gray" to enhance
...
class getImThread(QtCore.QThread):
    ...
    def run(self):
        ...
        origImg=mpimg.imread('Splash.png')  # Read splash image
        mean = 0.0                          # Noise stats for fake image...Zero mean
        sigma = 0.3                         # Std dev of noise
        row,col= origImg.shape              # Image dimensions

        nextImageTime = 0

        while self.running:                 # We are waiting for a name to start

            while nextImageTime > time.time():
                time.sleep(0.01)

            #time.sleep(self.iTime)          # Sleep for the interval

            #print "FLIR> Taking picture "

            if self.camConnected:           # Get real data!

                self.img = self.capture(False,"/dev/spidev0.1")         # Grab lepton frame

            else:
            
                self.img = origImg + np.random.normal(mean,sigma,row*col).reshape(row,col)   # Nice and compact!

            #### END FAKE IMAGES

            self.emit( QtCore.SIGNAL('GotOne(QString)'), datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S"))               # Inform calling routine

            self.nam = ""     # Reset name to blank to wait for the next name

            nextImageTime = time.time() + self.iTime
...
class AppWin(QtGui.QMainWindow):
    ...
    def initUI(self):               
        ...
        self.ui.iTime_le.setValue(0.4)                          # Default to 0.4 seconds
        #self.ui.iTime_le.setValidator(QtGui.QDoubleValidator())  # Float entry only!
        self.ui.iTime_le.valueChanged.connect(self.newItime)  # Grab new interval time
...
#------------------------------------------------------------------------------------------------#
    ...
    def drawHisto(self):
        ...
        self.histo.hist(self.theImg.flatten(), 15)       # plot Histogram
